...
def mainRun():
    ...
    #  데이터 컬럼만 뽑아내기 (복잡)
    userDataFrame = dataframe.iloc[::2, 2:3:] # 열 짝수로만 추출  (행선택),(열 선택)
    # print(userDataFrame)
...
def getEmailFromString(dataStr):
    ...
    # 오류 인스타: fream_official / 이메일: fream_official
    # 이메일 들어갈 자리에 인스타 또 들어감 인스타(윌비): wind2p / 이메일: smilewendylee@gmail.com     # 이메일 들어갈 자리에 인스타 또 들어감 인스타(윌비): wind2p / 이메일: smilewendylee@gmail.com
    # 이메일 연속 두개가 규칙없이 이어져 있음 이메일: yescoment@naver.com ahhn2019@gmail.com

    if("전화번호" in dataStr ):
        retrunStr = retrunStr[retrunStr.index("이메일") + 3:retrunStr.index("전화번호")]
    else:
        retrunStr = retrunStr[retrunStr.index("이메일")+3:]
